                ...
                <form method="post">
                    ...
                    $id = $_GET['id'];
                        if(isset($_POST['submit'])) {
                            ...
                            $ulasan = $_POST['ulasan'];
                            $rating = $_POST['rating'];
                            $query = mysqli_query($koneksi, "UPDATE ulasan_buku set id_buku='$id_buku', ulasan='$ulasan', rating='$rating' WHERE id_ulasan=$id");

                            if($query) {
                                echo '<script>alert("Ulasan Berhasil Diubah");</script>'; 
                            }else{
                                echo '<script>alert("Gagal Mengubah Buku");</script>';
                            }
                        }
                        $query = mysqli_query($koneksi, "SELECT*FROM ulasan_buku WHERE id_ulasan=$id");
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-8">
                            <select name="rating" class="form-control md-2">
                                <?php
                                    for($i = 0; $i<=10; $i++){
                                        ?>
